...
class LoginController extends GetxController {
  ...
  Future<void> checkUserRegistered() async {
    ...
    final snapshot = await ref.child("workers/${currentUser?.uid}").get();
    if (snapshot.exists) {
  ...
  void registerUser() async {
    ...
    DatabaseReference ref =
        FirebaseDatabase.instance.ref("workers/${currentUser?.uid}");
    await ref.set({
      "uid": currentUser?.uid,
      "mobileNumber": mobileNumber,
    }).then((value) {
      isLoading.value = false;
      Get.offAllNamed(AppRoutes.homeWorkerContainer1Screen);
    }).catchError((error) {
      isLoading.value = false;
      Get.snackbar(
        "Registration Failed",
